...
const MovieSlider = ({ category }) => {
  ...
  const scrollLeft = () => {
    if (sliderRef.current) {
        sliderRef.current.scrollBy({left: -(sliderRef.current.offsetWidth * 0.75), behavior: 'smooth'})
    }
  }
  const scrollRight = () => {
    if (sliderRef.current) {
  ...
  return (
    <div className="bg-black text-white relative px-5 md:px-20" onMouseLeave={() => setShowArrows(false)} onMouseEnter={() => setShowArrows(true)}>
